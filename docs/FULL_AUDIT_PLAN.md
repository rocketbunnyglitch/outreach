...
- [~] P300-P301 Watchdog force-fire: the dead-inbox alert's FIRST live run caught its own bug (role IN ('admin','superuser') — superuser not in staff_role enum {admin,lead,outreach,readonly}; 22P02, sweep failed non-fatal) → fixed role='admin'. Next hourly run (with 4-5 needs_reauth inboxes incl sergio) = the real validation; remaining rules (aging watchdog 5, bounce-rate) to force-fire after.
...
- [~] P316-P317 Analytics truth: emails-per-staffer verified CONSISTENT across drafts/messages/team-analytics for Jun-11 (Yesu 21-22, Gela 16, Bryle 1-4 — window edges); persona model confirmed (staffer OWNS alias inboxes: JC=massimo+marcus, Gela=micaela+angela, Yesu=daryl+adrian → 'JC sent 26' was the team's queue via Gela-owned micaela). Full metric-by-metric reconciliation remains.
