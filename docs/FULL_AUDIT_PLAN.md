...
- [ ] **P-PRIORITY: streamed-Suspense abort race (best lead yet for the iPhone freeze class).** LAB REPRODUCTION (2026-06-12, iframe rig): /inbox loaded in a same-origin iframe (390px AND 1200px — viewport NOT the variable) reliably ends with content stuck in React streaming staging divs: B:0/B:1 anchors present, S:0/S:1 hidden divs present, $RC("B:1","S:1") script PRESENT but failed (staging div not consumed), $RC("B:0","S:0") NEVER ARRIVED (stream aborted early). Page shows chrome only, zero visible threads, no self-recovery — matches the iPhone "frozen page" beacons ($R*/parentNode signature). THEORY: the inbox's early client-side history.replaceState (in-memory filter overlay) / router work fires while the stream is still open → Next aborts the rest of the stream → late Suspense content never relocates and React doesn't client-render the boundary. Desktop tabs win the race (fast assets); iframes (deprioritized loading) and phones on cell networks lose it. NEXT: (1) find the earliest client effect on /inbox that touches history/router; (2) gate it on stream completion (or post-hydration idle); (3) re-test in the iframe rig (deterministic repro = deterministic verification); (4) if confirmed, sweep other pages with early history.replaceState (data-table filter overlay is shared!). Repro recipe: inject same-origin iframe src=/inbox, wait 15s, check body > div[hidden][id^=S:].
