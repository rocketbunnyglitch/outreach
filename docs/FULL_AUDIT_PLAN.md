...
### W2 MOBILE REMAINDER — DONE 2026-06-14 (operator: "finish the mobile audit"). Full 390px iframe-rig overflow sweep of ~38 routes. Findings + fixes (all shipped): /admin/learning tables pushed page 195px → Section wraps children in overflow-x-auto (incl. new autonomy-readiness table); /admin/email-health 10-col account table 552px → overflow-x-auto + min-w-640; /admin/analytics header 70px → flex-col sm:flex-row stack; /me/activity header 59px → flex-wrap + px-4 sm:px-6; CRASH (latent, not overflow): /admin/analytics/<non-uuid> fell through to [staffId] route → 22P02 → guarded with UUID regex → notFound() (§12.3). All other ~33 routes 0-overflow at 373px CSS incl. detail pages (/venues/[id], /events/[id], /city-campaigns/[id]), all admin subpages, /misclassifications, /reference, /audit. Negligible: /crawl-management 9px shell artifact (no specific culprit, imperceptible). Commits b17f377 + cd59c5c. RIG NOTE: this run achieved true 373px CSS viewport (iframe at 390px width) — earlier passes were capped ~855px by Windows scaling; the iframe-into-logged-in-page rig bypasses that. Remaining (operator-phone only): touch-gesture ergonomics (kanban/gantt drag), popover anchor-direction at true phone width, keyboard input types — need a real device, not the rig.
### W2 MOBILE REMAINDER — (was PARKED 2026-06-12: "skip mobile for now, come back after")
...
- [x] P280-P281 Floors + overrides: cold cross-domain floor enforced live (JC's block = working as specified), warm exempt per refdoc 6.4 (@a6c7d84), capDeferred/floorDeferred dispositions shipped in P276; cold-override path instrumented (sendDraft entry logs gate-options — next occurrence is a one-grep diagnosis). Residual live-exercise is PASSIVE by design.
- [x] P282-P283 Reply ingestion CLOSED: match-rate 75.6% (unmatched = SaaS noise, gated @260539e); bounce extraction fixed (GMAIL_NOT_DELIVERED_RE); dead-inbox alert implemented + live-proven; history-gap fallback REVIEWED SOUND: history.list 404 → cursor nulled → first-poll backfill (in:inbox OR in:sent, 90d window, paged to 3,000-msg cap, dedupe via ON CONFLICT) → cursor restored from profile; admin deep-resync overrides window for full-archive needs.
- [x] P284-P285 Classification CLOSED @e0ce41e: was_override (never written → thread-level 45.9%) tightened to PER-MESSAGE comparison (classifier_runs.message_id is 100% populated); re-backfilled 1,277 rows → 39.8% true disagreement (78 rows were legitimate label drift, not AI error).
...
- [x] P296-P297 Health v2 CLOSED: command-center crash fixed (cron_name 42703) + render-verified (15 decision items); scope-403 → needs_reauth shipped+proven. Per-input verification: wristbandsPending EXACT (5 events with pending trackers = the 5 'need attention'; Edmonton correctly silent — no wristband-role slots → no trackers); stale-warm matches NBA definition (needs_reply interested/warm/question/callback >48h); sendingIssues = non-connected campaign inboxes (proven by the 5 needs_reauth); hosts/unowned event-week-only by design. Campaign chip 74-yellow consistent with 5 yellow crawls, 0 red.
...
- [x] P306-P307 Eventbrite DORMANT by reality: 0 linked events (listings not created yet); sync cron healthy on 15-min cadence; unlink-zeroes-sales fix + sales_without_eb_link invariant standing from earlier. RE-AUDIT TRIGGER: when the first listing links (full pull + venue-block marker verification then).
...
- [x] P314-P315 Auth matrix CLOSED: (admin) group layout calls requireStaff() — every page auth-gated even without inline guards (the 10 'unguarded' pages are covered); middleware does session gate + role-aware landing; sensitive ACTIONS verified admin-gated (autonomy hasMinimumRole-admin, suppression + users requireAdmin per action; autonomy dispatch additionally hard-requires AUTONOMY_DISPATCH_ENABLED, unset). Machine routes key/secret-verified live. Residual: interactive login-as-readonly/outreach walk needs test accounts — optional, flagged.
...
- [x] P348 Hosts: surfaces exist + render (Internal/External hosts pages in route sweeps; crawl-support gantt); two-host rule wired in health (event-week) + watchdog (hostless P1-3 <7d); ALL data tables empty pre-season (internal/external/crawl_hosts/shipments 0) — roster population is October-run-up operator work. SMS consent inert without provider (Twilio SKIP-FLAG); host-payments page (refdoc 7.13.7) NOT BUILT — already on the reported backlog. In-season e2e re-audit when hosts onboard.
- [x] P349 Guest-count math VERIFIED AGAINST SOURCE: lib/turnout-quote.ts INITIAL_PITCH_NUMBER matches refdoc §5.2 EXACTLY all 6 priority rows (P1 200-300/100-200/100-200 with documented inline-tail deviations; P2-P6 verbatim incl the P5-6 shared row); sales-update buckets per §5.3-5.4 (round-down boundaries, 70% deflation >150, honest-slow flag <20); 13 unit tests; live-exercised in the P288 merge-field smoke.
- [x] P350 Handoff/escalation: cold cross-domain floor enforced at dispatch (proven live via JC's blocked send — working as specified), warm conversations exempt per 6.4 (@a6c7d84); handoff ORDER BY last_message_at fixed in security wave; handoff-next hint on cold table + escalation-status popover (clamped for mobile) shipped in refdoc purpose-fit wave; relationship gate universal at dispatch. Flow pieces individually verified; full two-brand live handoff awaits second-brand traffic (none yet — single OutreachBrand active).
- [x] P351 Wristband logistics: tracker LIVE with 5 pending Calgary trackers (auto-created by confirm cascade incl 4-row backfill from A2), feeding health (soft-reason any distance, blocker in event week) + command-center rows + aging-watchdog <14d rule; /wristbands renders clean in sweeps. Shipment-marking + alert escalation get natural live verification as shipping season approaches.
- [x] P352 Enrichment: LIVE-USED — 238 attempts, status spread sane (99 tier1_success / 10 partial / 86 tier2_failed / 39 unreachable; 6,084 never-attempted); skip logic verified in code: decideEligibility(latestAttempt) gates re-attempts unless forceRetry, attempt row opened up-front so crashes leave a trace; per-domain cooldown + jitter (polite crawler).
