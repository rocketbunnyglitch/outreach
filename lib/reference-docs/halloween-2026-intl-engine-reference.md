...
0. [Foundational principles](#0-foundational-principles)
   ...
   - 0.5 Outreach is human-operated and one-to-one
...
### 0.5 Outreach is human-operated and one-to-one [LOCKED]

Outreach is human-operated and one-to-one (not bulk automated marketing): the engine drafts, a human reviews and sends every message. The working assumption has been that this posture does not require country-specific opt-out boilerplate in the message body.

Compliance posture should be confirmed with legal; see docs/REFERENCE_DOC_GAPS.md (fuller discussion in docs/COMPLIANCE_NOTES.md). This is not a legal determination.
...
> Implementation status (effective-priority computation, worklist wiring): see docs/IMPLEMENTATION_STATUS.md (section 1.6).
...
> Implementation status (venue_domain_relationships storage, auto-detect, hard block, decay): see docs/IMPLEMENTATION_STATUS.md (section 3.3). Known gap (scheduled-send runner relationship re-check): docs/REFERENCE_DOC_GAPS.md.
...
Outreach is human-operated and one-to-one (the engine drafts, a human reviews and sends), not bulk automated marketing. The working assumption has been that this posture does not require country-specific opt-out boilerplate in the message body.
...
> Implementation status (where the pitch number is surfaced -- T3/T4 merge fields, pre-booking fallback): see docs/IMPLEMENTATION_STATUS.md (section 5.7). The business rule that slot-type x priority drives the number is section 5.6 above.
...
> Implementation status (cold-outreach Handoff button, 7-day floor check, brand re-attribution): see docs/IMPLEMENTATION_STATUS.md (section 6.2).
...
> Send pacing (cold-send cooldown + email queue) and how open slots are written in openers are engine behaviors, not business rules. Both still hold to the human-written / human-reviewed principle (0.4) and the cadence floors + hard cap above; pacing only controls WHEN a reviewed message leaves the inbox, never what it says. Implementation status: see docs/IMPLEMENTATION_STATUS.md (sections 6.9, 6.10).
...
> Implementation status (lifecycle scheduler, touch timing -- T9/T9-near/T11/T13/T13W/T14/T15/T17, idempotency, multi-night anchoring, and the week-out T13W re-anchor): see docs/IMPLEMENTATION_STATUS.md (section 7.1). Known gaps (per-night T15 split, late-addition T9-near collapse, ticketing-dependent bundle): docs/REFERENCE_DOC_GAPS.md.
...
> Implementation status (host briefing scheduler H0a/H0b, merge fields, idempotency): see docs/IMPLEMENTATION_STATUS.md (section 7.13.9).
...
> Implementation status (cancellation flow -- cancel, stop-downstream, T16 draft, city-lead notify): see docs/IMPLEMENTATION_STATUS.md (section 7.16). Known gaps (acknowledgment tracking + auto-escalation, comeback flow, misrouted-positive-reply routing, cancelled-venues view): docs/REFERENCE_DOC_GAPS.md.
...
**Inbox ordering principle.** The inbox should keep "what needs my attention" -- i.e. who replied -- at the top, instead of the operator's own outbound churn reshuffling the list. (Implementation -- Gmail-style ordering, bumped only on inbound reply: see docs/IMPLEMENTATION_STATUS.md, section 8.2.)
...
Moved to ROADMAP.md, under "Phase 2 engine build items (moved from reference doc section 11)". These are roadmap/status items, not canonical business rules; the rule each one implements lives in sections 1-10 above. See also docs/IMPLEMENTATION_STATUS.md for what has shipped.
...
> Note: the former "Engine current behavior -- 2026-06 build" section (which mirrored LOCKED rules into shipped engine behavior) has moved to docs/IMPLEMENTATION_STATUS.md so this canonical doc holds only business rules. That file maps each business section to its implementing code; known gaps are in docs/REFERENCE_DOC_GAPS.md.
