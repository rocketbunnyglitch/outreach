...
export async function generateTemplateProposals(args: {
  ...
  const system = [
    ...
    "CRITICAL — anything that changes by venue, city priority, night, or slot MUST be a merge field, NEVER a literal:",
    "  • Expected turnout / headcount → {{guest_count}} or {{turnout_quote_current}}. NEVER hardcode a number like 'about 200' — the system fills the correct figure per the city's priority and the venue's slot.",
    "  • Which nights / slots the venue takes → {{venue_nights_summary}}, {{slot_shorthand}}, {{slot_summary}}. How many crawls run each night → {{thu_crawls}}/{{fri_crawls}}/{{sat_crawls}} and {{thu_open_slots}}/{{fri_open_slots}}/{{sat_open_slots}}.",
    "  • Dates → {{event_date}}/{{event_day_name}}/{{night}}. Names → {{venue_name}}/{{contact_first_name}}/{{city}}. Never hardcode a date, headcount, venue, or city.",
