...
## #022 — Staff picks brand/alias at send time, not at campaign creation

- **Date:** Phase 11 build
- **Decision:** Campaigns are NOT pinned to an outreach brand. When a staffer drafts/sends a cold email, they pick which of their connected aliases to send from. Each staffer may have aliases across multiple brands.
- **Context:** Original model coupled each campaign to one `outreach_brand_id`. The operator clarified: a single campaign (e.g. St. Paddy's 2026) gets outreach from multiple brands. Per-campaign brand was a fiction.
- **Schema impact:**
  - DROP `campaigns.outreach_brand_id` FK + column
  - DROP `city_campaigns.outreach_brand_id` if it exists
  - KEEP `outreach_brands` table — brands still exist as containers for aliases
  - The `staff_outreach_emails` table is the canonical join: each row already references both a `staff_member_id` AND an `outreach_brand_id` (the alias's brand)
- **UI impact:** Send composer picks alias from `staff_outreach_emails WHERE staff_member_id = me`. AI draft + inbox already operate per-alias; no change there.
- **Status:** active

---

## #023 — `crawl_brand` removed entirely (no campaigns have run)

- **Date:** Phase 11 build
- **Decision:** Drop the `crawl_brands` table + all references. The two-brand-type model (CrawlBrand public-facing + OutreachBrand operational) is replaced with a single `outreach_brands` table.
- **Context:** Operator: "no campaigns have been ran so removing brand is fine." The crawl-brand concept was for the public-facing crawl pages — but per #024 below, public pages aren't a sub-domain of this app, they live in a separate www/ folder on a different deployment. So the CrawlBrand concept doesn't need to live in this database at all.
- **Schema impact:**
  - DROP `crawl_brands` table
  - DROP `campaigns.crawl_brand_id` column
  - DROP any other FKs to `crawl_brands`
  - Migration is safe because no campaigns have been finalized yet (operator-confirmed)
- **Status:** active

---

## #024 — Public crawl pages live OUTSIDE this app

- **Date:** Phase 11 build
- **Decision:** The public-facing crawl event pages (where customers buy tickets / RSVP) are NOT subdomains of `outreach.barcrawlconnect.com` and are NOT generated by this Next.js app. They are static HTML in a separate `www/` folder on a different deployment.
- **Schema impact:** DROP `campaigns.public_subdomain` column. Remove the field from the campaign editor.
- **Operator quote:** "We do not need public facing subdomain in the campaign menu... Public pages will not be a sub domain they will be html pages that should be on another domain so will be on a different www/ folder."
- **Status:** active

---

## #025 — Goals: outreach-side (cities scheduled) vs admin-side (ticket sales)

- **Date:** Phase 11 build
- **Decision:** Two distinct goal types, separately permissioned:
  - **Outreach goals** (visible to all outreach staff on the campaign dashboard): `target_cities_scheduled` filtered by `max_priority` tier. Example: "schedule 50 cities at P1-P4 for St. Paddy's 2026." NO dollar values surface anywhere outreach staff can see.
  - **Admin-only goals** (visible only to admin role at `/admin/goals`): `target_ticket_sales_count` (per-city avg or total). This is the COUNT of tickets sold, NOT cents.
- **Drops:** `campaigns.revenue_goal_cents`, `campaigns.venue_count_goal`. Both replaced.
- **Context:** Operator: "the goals are sales not in cents but in ticket sales... outreach goals should be target number of cities to schedule (out of specific priority for examples 50 cities to be scheduled that are priority 1,2,3, or 4)... goals should only be seen by admin so classified under admin." Outreach staff have ZERO visibility into revenue or dollar figures by design.
- **Schema impact:**
  - ADD `campaigns.target_cities_scheduled INT NULL`
  - ADD `campaigns.max_priority_for_scheduling SMALLINT NULL DEFAULT 4`
  - ADD `campaigns.target_ticket_sales_count INT NULL`
  - DROP `campaigns.revenue_goal_cents`
  - DROP `campaigns.venue_count_goal`
- **Status:** active

---

## #026 — CSV city import uses fuzzy match with review row when ambiguous

- **Date:** Phase 11 build
- **Decision:** Bulk-add cities to a campaign via CSV. Each row matches against `cities.name`:
  - **Auto-accept** when match confidence is unambiguous (case-insensitive exact, whitespace-trim, optional region/state match, or a single Levenshtein-distance-1 hit). Operator never sees these.
  - **Review required** when ambiguous (e.g. "Springfield" matches 4 cities). Render a sticky review banner: "We found N rows that need confirmation" with a one-tap "Accept this suggestion" or "Pick a different city" per row.
  - **Hard fail** (skip + log) when zero matches.
- **Operator quote:** "accept loose in q5 or just an accept this suggestion"
- **CSV columns:** `city`, optional `priority`, optional `region`. Headers case-insensitive.
- **Status:** active

---

## #027 — Send caps are per alias, not per staffer

- **Date:** Phase 11 build
- **Decision:** The daily send cap (currently 30-40/day default) is enforced on EACH `staff_outreach_emails` row, not aggregated per staffer.
- **Context:** Bryle is assigned 3 aliases (`josh@brand1.com`, `chris@brand1.com`, `evan@brand1.com`). His effective daily cap is 3 × the per-alias limit. Gmail's per-account warm-up + reputation lives at the alias level; cross-alias aggregation would defeat that.
- **Operator quote:** "the 40 per day are per alias/email account right thats the limit"
- **Schema impact:** None — the existing `staff_outreach_emails.daily_send_limit` column already lives at the alias level. The `effectiveDailyCap()` helper in `lib/send-throttle.ts` already reads from this row. The change is to documentation + the send-cap pill UI, which should show the **sum** across the staffer's aliases when collapsed, and per-alias breakdown when expanded.
- **Status:** active
