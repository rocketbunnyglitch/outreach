...
export default async function TasksPage({ searchParams }: TasksPageProps) {
  ...
  return (
    <div className="flex animate-[fade-in_300ms_ease-out] flex-col gap-8">
      ...
      <form
        method="get"
        className="flex flex-wrap items-end gap-3 rounded-lg border border-stone-200 bg-white p-4 dark:border-stone-800 dark:bg-stone-950"
      >
        ...
        {(params.status || params.assignee || params.due) && (
          <Link href="/tasks" className="text-stone-500 text-xs underline-offset-2 hover:underline">
            Clear all
          </Link>
        )}
      </form>
            ...
            {rows.map((task, idx) => {
              ...
              return (
                <Link
                  ...
                  href={`/tasks/${task.id}`}
                  className={cn(
                    "grid grid-cols-12 items-center gap-3 px-4 py-3 transition-colors hover:bg-stone-100 dark:hover:bg-stone-900",
                    idx % 2 === 1
                      ? "bg-stone-50/50 dark:bg-stone-950"
                      : "bg-white dark:bg-stone-950/40",
                  )}
                >
                  ...
                  <div className="col-span-2 truncate text-sm text-stone-600 dark:text-stone-400">
                    {task.assigneeName ? (
                  ...
                  <div
                    className={cn(
                      ...
                      overdue ? "font-medium text-rose-500" : "text-stone-500",
                    )}
                  >
                    {task.dueAt ? (
                      <span className="inline-flex items-center gap-1.5">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-stone-200 border-t pt-4 dark:border-stone-800">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                ...
                className="rounded-md border border-stone-300 px-3 py-1.5 text-xs hover:bg-stone-100 dark:border-stone-700 dark:hover:bg-stone-900"
              >
                ← Previous
              </Link>
            )}
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    ...
    cancelled: "bg-stone-500/10 text-stone-400 ring-stone-500/20",
  };
